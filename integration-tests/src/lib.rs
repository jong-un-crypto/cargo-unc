...
pub mod from_git {
    ...
    pub const SDK_REPO: &str = "https://github.com/unc/unc-sdk-rs.git";
    ...
    pub const SDK_VERSION_TOML_TABLE: &str = formatcp!(
        r#"
        version = "{SDK_VERSION}"
        git = "https://github.com/unc/unc-sdk-rs.git"
        rev = "{SDK_REVISION}"
        "#
    );
}
...
macro_rules! invoke_cargo_unc {
    ($(Cargo: $cargo_path:expr;)? $(Vars: $cargo_vars:expr;)? Opts: $cli_opts:expr; Code: $($code:tt)*) => {{
        ...
        let cargo_unc::CliOpts::Near(cli_args) = cargo_unc::Opts::try_parse_from($cli_opts)?;

        match cli_args.cmd {
            Some(cargo_unc::commands::CliNearCommand::Abi(cmd)) => {
                let args = cargo_unc::commands::abi_command::AbiCommand {
                    no_doc: cmd.no_doc,
                ...
                cargo_unc::commands::abi_command::abi::run(args)?;
            },
            Some(cargo_unc::commands::CliNearCommand::Build(cmd)) => {
                let args = cargo_unc::commands::build_command::BuildCommand {
                ...
                cargo_unc::commands::build_command::build::run(args)?;
            },
            Some(_) => todo!(),
            None => ()
        }
...
macro_rules! generate_abi_with {
    ($(Cargo: $cargo_path:expr;)? $(Vars: $cargo_vars:expr;)? $(Opts: $cli_opts:expr;)? Code: $($code:tt)*) => {{
        ...
        let result_dir = $crate::invoke_cargo_unc! {
