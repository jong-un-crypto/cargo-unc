...
use unc_cli_rs::commands::account::create_account::sponsor_by_faucet_service::{
...
pub fn random_account_id(
    networks: &linked_hash_map::LinkedHashMap<String, unc_cli_rs::config::NetworkConfig>,
) -> color_eyre::eyre::Result<unc_cli_rs::types::account_id::AccountId> {
    loop {
        let mut generator = Generator::default();
        let random_name = generator.next().wrap_err("Random name generator error")?;
        let account_id =
            unc_cli_rs::types::account_id::AccountId::from_str(&format!("{random_name}.testnet"))?;
        if !unc_cli_rs::common::is_account_exist(networks, account_id.clone().into()) {
